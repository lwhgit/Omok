        ...
        while(True):
            ...
            for i in range(0, self.epoch):
                ...
                while(gameOver != True):
                    action = -9999
                    reward = 0
                    currentState[0, ...] = self.getState(currentPlayer, 0)
                    ...
                    for ki in range(1, 4):
                        currentState[ki, ...] = self.getState(currentPlayer, ki)

                    result = self.omok.putStone(x, y, currentPlayer)

                    for ki in range(0, 4):
                        nextState[ki, ...] = self.getState(currentPlayer, ki)
                    ...
                    for kj in range(0, 4):
                        memory.remember(currentState[kj], action, reward, nextState[kj], gameOver)
...
class replayMemory:
    ...
    def getBatch(self, model, batchSize, winReward, sess):
        ...
        for i in range(0, chosenBatchSize):
            ...
            target = sess.run(model, feed_dict = {X: current_inputState, DropoutRate: 0.8, DropoutHiddenRate: 0.5})
